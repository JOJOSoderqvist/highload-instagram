...
Для локальной балансировки нагрузки логичнее использовать L7 балансировщик (например, Envoy[^16]), так как он позволяет балансировать нагрузку по типу запроса,
что критически важно для сервиса с большой долей запросов на статический контент. Также он имеет возможность терминировать SSL, что упростит обработку запросов на конечных серверах.
...
[^16]: [Envoy](https://www.envoyproxy.io/docs/envoy/latest/intro/what_is_envoy)