...
Если запрос не содержит Partition key, координатор не может однозначно определить ноду, на которой могут находиться данные.
В нашем случае, используется NetworkTopologyStrategy, при такой стратегии все узлы кластера могут считаться потенциальными кандидатами
(нодами, содержащими нужные нам данные).
