...
1. Feed Service
   ...
   1. Feed getter недоступен - отдаем пустую ленту.
...
![Project scheme](diagrams/hw-10-fix-dlq.svg)
...
| База данных | QPS(*)    | CPU | RAM  | Disk   |
|-------------|-----------|-----|------|--------|
| Cassandra   | 486 900   | 98  | 980  | 471 TB |
| Redis       | 2 424 200 | 485 | 4850 | 1 TB   |
...
Для датацентра с нагрузкой 8% - 1420 CPU, 138 GB RAM и 38 TB Disk Storage. С учетом потенциального прироста пользователей и возможность держать дополнительную нагрузку
...
|-------------------------------------------------------------------------------|------|------------------|--------------|
| 1 x Intel Xeon Silver 4416+ 20 Cores/4 x 16GB RAM/ 1x NVME 1T / 1 x 10 Gbit/s | 1640 | 82               | 4500€        |


Итого общая стоимость - 82 * 4500 € = 369 000 €, годовая амортизация (20%): 369 000 € / 5 лет = 73 800 €/год,
Месячная амортизация: 73 800 € / 12 месяцев = 6150 €/месяц на все серверы, 75 €/месяц на один сервер.

#### Вариант для 7%

Для датацентра с нагрузкой 7% - 1243 CPU, 121 GB RAM и 34 TB Disk Storage. С учетом потенциального прироста пользователей и возможность держать дополнительную нагрузку
при сбое одного из соседних ДЦ получаем:
...
| Конфигурация                                                                  | Ядра | Количество на ДЦ | Цена за 1 шт |
|-------------------------------------------------------------------------------|------|------------------|--------------|
| 1 x Intel Xeon Silver 4416+ 20 Cores/4 x 16GB RAM/ 1x NVME 1T / 1 x 10 Gbit/s | 1500 | 75               | 4500€        |


Итого общая стоимость - 75 * 4500 € = 337 500 €, годовая амортизация (20%): 337 500 € / 5 лет = 67 500 €/год,
Месячная амортизация: 67 500 € / 12 месяцев = 5625 €/месяц на все серверы, 75 €/месяц на один сервер.
